...
- **Icons**: brand/tech-stack icons in `Skills.jsx` are individual SVG files in `src/assets/` — leave these as-is. Every other icon on the site (mail in `Contact.jsx`, theme toggle and hamburger in `Navbar.jsx`, the logo sparkle, project/service badges) comes from `react-icons/lu` (Lucide) for one consistent line-icon style. Prefer `lu` icons for any new non-brand icon; only reach for another react-icons set if Lucide is missing the glyph.
- **Social links**: the six profile links shared by `Contact.jsx` (white tiles) and `Footer.jsx` (bare icons) live in `src/data/socials.jsx` — edit URLs there, not in the components. Their brand glyphs come from `react-icons/fa6`, not Lucide: Lucide has no TikTok mark and Simple Icons no longer ships LinkedIn, so `fa6` is the only set in play covering all of them. GitHub uses `FaGithub` here rather than `assets/github.svg` (which `Skills.jsx` still uses) because a font icon inherits `currentColor` — the footer's untiled icons need that to stay visible in dark mode and to hover purple.
